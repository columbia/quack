...
def do_analysis(project_path, result_path):
    analyzer = PHPAnalyzer(project_path=project_path, results_path=result_path)
...
def compare_call_result(expected: dict, actual: dict) -> bool:
    ...
    if all_expected != all_actual:
        print_failure(
            expected,
            actual,
            f"Allowed types and classes mismatch, difference: {all_expected.symmetric_difference(all_actual)}",
        )
        return False
    ...
    if expected_types != actual_types:
        print_failure(
            expected,
            actual,
            f"(Test still passes) allowed types mismatch, difference: {expected_types.symmetric_difference(actual_types)}",
        )
    ...
    if expected_classes != actual_classes:
        print_failure(
            expected,
            actual,
            f"(Test still passes) Allowed classes mismatch, difference: {expected_classes.symmetric_difference(actual_classes)}",
        )
